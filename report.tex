...
otherwise the stiffness matrix is singular at $\omega = 0$.

\textbf{Phase~I (PPF model):} four bottom perimeter edge lines carry a Fixed
Constraint (geometric entity level set to \emph{Edge}).  This is physically
correct for the clamped-perimeter assumption and gives $f_1 \approx 214$~Hz.
Base excitation (Study~2) is not run on this model.

\textbf{Phase~II (IMU model):} a Domain Spring Foundation on the full plate domain
provides realistic foam-bed mounting ($f_\text{rigid} \approx 1.7$~Hz).  This is
the only boundary condition that is consistent between Study~1 and Study~2 AND can
be driven by the Base Excitation node.  A Boundary Spring Foundation connects to a
fixed reference frame and contradicts the Base Excitation prescription, producing a
flat non-physical $H_{11}$.  Rigid Motion Suppression gives different eigenmodes
than the spring foundation, making the two studies inconsistent.
...
force, which is the definition of a damping force.
...
\begin{tikzpicture}[
  ...
  \node[block] (Kppf) at (4,-1.6) {$K_{\mathrm{PPF}}$};
...
$-1$ for any finite positive gain $g$, giving unconditional stability for
collocated PPF.
...
\begin{tikzpicture}[
  ...
  \node[block] (H22)  at (4,0)    {$H_{22}$};
  \node[block] (Kdvf) at (4,-1.6) {$K_{\mathrm{DVF}}$};
...
\subsection{State-Space Model and Time-Domain Simulation}
\label{sec:ssest}

The COMSOL output is a frequency-domain FRF sampled at discrete frequencies.  To
run time-domain simulations (impulse response, sine sweep with DVF on/off), a
continuous-time state-space model is fitted to the FRD data.

\paragraph{What a state-space model is.}
A state-space model describes a dynamic system as:
\begin{align}
  \dot{\mathbf{x}}(t) &= A\mathbf{x}(t) + B u(t), \label{eq:ss_state} \\
  y(t) &= C\mathbf{x}(t) + D u(t). \label{eq:ss_out}
\end{align}
The vector $\mathbf{x}$ is the internal state (here: displacement and velocity of
the plate at the target mode), $u$ is the input (base acceleration or actuator
voltage), and $y$ is the output (IMU acceleration).  For a single resonance the
minimum state dimension is $n_x = 2$ (one complex pole pair).

\paragraph{Fitting with \texttt{ssest}.}
MATLAB's \texttt{ssest} fits $A$, $B$, $C$, $D$ to the measured FRD by
minimising the prediction error between the model frequency response and the
COMSOL data.  The fit is restricted to a narrow frequency window
($f_1 \pm 40$~Hz) so that only the target mode is fitted; fitting the full
spectrum would force the model to compromise across multiple resonances and produce
unphysical poles.

\begin{verbatim}
nx = 2;   % one complex pole pair = one resonance
LTI_OL = ssest(idfrd(H21(mask), w(mask), 0), nx, opt_ss);
\end{verbatim}

\paragraph{Verification.}
After fitting, \texttt{damp(LTI\_OL)} prints the natural frequency and damping
ratio of each pole.  Expected values: $f_n \approx f_1$, $\zeta \approx 0.01$
(half the loss factor $\eta = 0.02$ used in COMSOL).  A pole far from $f_1$ or
with $\zeta \ll 0.01$ indicates a non-physical fit, usually caused by fitting
...
\texttt{H11.txt}, \texttt{H12.txt}, \texttt{H21.txt}, \texttt{H22.txt}:
tip displacement and sensor voltage, frequency range 10--1000~Hz.
...
% -----------------------------------------------------------------------
\section{Conclusions and Applicability to the ASML Gridplate}
\label{sec:conclusions}

\subsection{Conclusions from This Model}

\begin{itemize}
  \item \textbf{PPF (Phase~I):} With a collocated DuraAct piezo sensor and $g = 0.3$,
        $\zeta_f = 0.3$, the predicted suppression at $f_1 \approx 214$~Hz is
        $\approx -22.6$~dB.
        \emph{[TODO: replace with actual COMSOL closed-loop result once H11/H12/H21
        are re-exported.]}

  \item \textbf{DVF (Phase~II):} Mode~3 at 160.90~Hz shows a clean $-90^\circ$
        phase crossing and is the reliable DVF target.  At $g = 0.30\,g_\text{max}$
        the predicted suppression is approximately $-10$ to $-15$~dB, consistent
        with the Falangas~\cite{falangas1994} benchmark of 10.5~dB.
        \emph{[TODO: replace with exact dB value from dvf\_main.m once
        H11 and H12 are re-exported from COMSOL.]}

  \item \textbf{Mode~1 at 86.51~Hz is not reliably controllable} with simple DVF
        from the current patch and IMU positions.  The phase of $H_{22}$ near this
        mode is feedthrough-distorted, which means the sign assumption of the
        controller breaks down locally.

  \item \textbf{Domain Spring Foundation} is required for a consistent MIMO matrix.
        Rigid Motion Suppression or a Boundary Spring Foundation both produce
        incorrect $H_{11}$ when combined with base excitation.
\end{itemize}

\subsection{Applicability to the ASML Gridplate}
\label{sec:gridplate}

\paragraph{What is the gridplate?}
The ASML gridplate is a large flat reference plate (typically glass-ceramic or
aluminium) that carries a precision encoder grating.  Encoder heads on the wafer
stage read position from this grating, so any vibration of the gridplate directly
degrades positioning accuracy.  Position stability requirements are on the order of
0.15~nm over 24~hours~\cite{gridplate_patent}.  Stage accelerations and
decelerations excite gridplate bending modes, which ASML literature places in the
range of 100--300~Hz (horizontal modes near 160--180~Hz, vertical modes near
250--300~Hz).  These frequencies overlap directly with the modes observed in this
simulation (160.90~Hz, 225.56~Hz).

\paragraph{Can DuraAct piezo patches control gridplate vibrations?}
Based on the findings of this project, the following questions need to be answered
before a conclusion can be drawn.  This section is a \textbf{template} for the
next modelling step.

\begin{enumerate}
  \item \textbf{Plate size and material.}
        The dummy plate used here is $1000 \times 1000 \times 100$~mm aluminium.
        A real ASML gridplate is likely thinner and may use Zerodur or a
        glass-ceramic composite.  Modal frequencies scale with thickness and
        elastic modulus.  \emph{[TODO: obtain gridplate dimensions and material
        from ASML documentation and re-run the eigenfrequency study.]}

  \item \textbf{Actuator authority.}
        The DuraAct patch ($50 \times 30$~mm, d31 mode) applies a bending moment
        over a small footprint.  On a 1~m plate this patch couples well into the
        first mode.  For a larger or stiffer gridplate the coupling coefficient
        $A_n$ in~\eqref{eq:H22ppf} will decrease.  A lower $A_n$ means a lower
        $H_{22}$ peak and a smaller stable gain range.
        \emph{[TODO: compute $H_{22}$ peak magnitude for the gridplate geometry
        and compare with the dummy plate value of $\approx 0.17$~m/s$^2$/V.]}

  \item \textbf{Multiple modes in the target band.}
        This model shows four flexible modes between 87 and 226~Hz.  A larger,
        thinner gridplate may have more closely spaced modes.  DVF with a single
        patch will attempt to damp all modes simultaneously; whether the gain margin
        is sufficient across all modes requires a Nyquist check on the full
        frequency-domain FRF.

  \item \textbf{Spatial coverage.}
        A gridplate that must remain flat over its entire area needs spatially
        distributed actuation.  A single patch suppresses vibration locally near
        the patch; the plate may still vibrate at antinodes far from the patch.
        Multiple patches and a decentralised control architecture (one
        DVF/PPF loop per patch) are likely needed, as demonstrated by
        Luleci~\cite{luleci2013} for a $700 \times 400$~mm plate.

  \item \textbf{Environmental constraints.}
        ASML lithography machines operate in a controlled environment.  Factors
        such as vacuum compatibility, thermal stability, outgassing, and
        electromagnetic interference from the encoder system may restrict which
        patch materials and amplifier topologies can be used.
        \emph{[TODO: verify DuraAct P-876.A12 suitability for the ASML
        operating environment.]}
\end{enumerate}

\paragraph{Preliminary assessment.}
The simulation results show that a single DuraAct patch can suppress the first
bending mode of a 1~m aluminium plate by 10--22~dB depending on the controller
and sensor choice.  This level of suppression is physically achievable on a
comparable gridplate provided the actuator is placed at a high strain-energy
location and the target mode has a clean $-90^\circ$ phase crossing in $H_{22}$.
The main open question is whether the patch authority remains sufficient on a
stiffer or larger plate and whether a single-patch approach provides adequate
spatial coverage.
...
\bibitem{gridplate_patent}
ASML Netherlands B.V.,
``Lithographic apparatus and device manufacturing method,''
US Patent 10,551,751, 2020.
